...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product bg-color ">
      <div>
    <img class="product-image" src=${image}></img>
      </div>
      <h3><span class="title-color">${product.title}</span></h3>
      <p>Category: ${product.category}</p>
      <h2>Price: $ ${product.price}</h2>
      <h3><i style="color:orange" class="fas fa-star"></i>${product.rating.rate}</h3>
      <h5>Rated by <span class="text-danger">${product.rating.count}</span> people.</h5>
...
const updateTotal = () => {
  ...
  const grandTotal = price + delivery + tax;
